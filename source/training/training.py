...
class Train:
    ...
    def __init__(self, cfg: DictConfig,
        ...
        if self.config.model.name == 'RelationalBrainNetworkTransformer' or self.config.model.name == 'RelationalBrainNetworkTransformer2' or self.config.model.name == 'RelationalBrainNetworkTransformer3':
            ...
            self.epochs = cfg.training.epochs
        elif self.config.model.name == 'RelationalBrainNetworkTransformer4': 
            with open_dict(cfg):
                cfg.training.epochs = 10
            self.epochs = cfg.training.epochs
    ...
    def test_pairs_per_epoch(self, cfg, validation_dataloader, loss_meter):
        label_1 = []
        label_2 = []
        combined = []
        result = []
    ...
    def saving_criteria(self, label_1, label_2, combined, predicted):
        
        x, y = label_1.shape[0], label_1.shape[1]
        
        label_1 = label_1.reshape(x*y, 1)
        label_2 = label_2.reshape(x*y, 1)
        combined = combined.reshape(x*y, 4)
        predicted = predicted.reshape(x*y, 4)
        ...
        #return (np.mean(np.abs(L1-label_1)) + np.mean(np.abs(L2-label_2))) / 2.0
        return [np.mean(np.abs(L11-label_1)), np.mean(np.abs(L21-label_2))]
    
    def train_pairs_per_epoch2(self, cfg, train_dataloader, optimizer, lr_scheduler):
        self.model.train()
        ...
        for pair in paired_dataloader:
            ...
            # Concatenate r1, r2 into combined_label
            combined_label = torch.cat((r1, r2), dim=1)

            lr_scheduler.update(optimizer=optimizer, step=self.current_step)
            node_feature1, node_feature2, combined_label = node_feature1.cuda(), node_feature2.cuda(), combined_label.cuda()
            r1, r2 = r1.cuda(), r2.cuda()
            ...
            mae_error = mae(predict[:, :1], r1)
            self.train_accuracy.update(mae_error/2.0)
            
    def test_pairs_per_epoch2(self, cfg, validation_dataloader, loss_meter):
        label_1 = []
        label_2 = []
        combined = []
        result = []
        ...
        for _, node_feature1, label1 in validation_dataloader:
            for _, node_feature2, label2 in validation_dataloader:
                ...
                # Concatenate r1 and r2 into combined_label
                combined_label = torch.cat((r1, r2), dim=1)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            if self.config.model.name == 'RelationalBrainNetworkTransformer' or self.config.model.name == 'RelationalBrainNetworkTransformer2' or self.config.model.name == 'RelationalBrainNetworkTransformer3' or self.config.model.name == 'RelationalBrainNetworkTransformer4':
                self.train_pairs_per_epoch(self.config, self.train_dataloader, self.optimizers[0], self.lr_schedulers[0])
                val_result = self.test_pairs_per_epoch(self.config, self.val_dataloader, self.val_loss)
                flag = 1
            else:
            ...
            if flag == 1:
                overall_mae = self.saving_criteria(val_result['label1'], val_result['label2'], val_result['combined_scores'], val_result['predicted_score'])

                if np.mean(overall_mae) < best_val_loss:
                    best_val_loss = np.mean(overall_mae)
                ...
                self.logger.info(" | ".join([
                    ...
                    f'val Acc - L1:{overall_mae[0]: .3f}',
                    f'val Acc - L2:{overall_mae[1]: .3f}'
                ]))
            else:
                val_predictions = val_result
                self.logger.info(" | ".join([
        ...
        saving_path = '/neuro/labs/grantlab/users/arafat.hussain/Data/prime24'
        np.save(f"{saving_path}/New_{self.config.model.name}_{self.config.score}_bnt_{self.fold}_results.npy", val_predictions)
